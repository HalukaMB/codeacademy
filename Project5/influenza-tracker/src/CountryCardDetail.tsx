...
const CountryCardDetail = () => {
    ...
    const { reducedData, countryFilter } = useContext(ReducedDataContext) as GetDataProps
                        ...
                        ? <>
                            <div className="dotGrid">
                                {countryData.matrixDots.flat().map((element, index) => {
                                    if (element == "healthy") {
                                        return (
                                            <div className="circle healthy" key={index} data-index={index} onClick={changeCircle} />
                                        )
                                    }
                                    if (element == "infected") {
                                        return (
                                            <div className="circle infected" key={index} data-index={index} onClick={changeCircle} />
                                        )
                                    }
                                })}
                            </div>
                            <div className="explanation">* We assume there is the chance of at least one person being infected.</div>
                        </>
